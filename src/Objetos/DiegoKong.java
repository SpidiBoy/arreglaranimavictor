...
public class DiegoKong extends GameObjetos {
    ...
    // 🌟 NUEVA CONSTANTE PARA EL SPRITE DE 48x40
    private static final float HEIGHT_AGARRA_PRINCESA = 40;
    ...
    public void render(Graphics g) {
        if (animacionActual != null) {
            ...
            // ==================== CAMBIO CLAVE ====================
            // Determinar las dimensiones y la posición Y correctas para renderizar
            // en función del estado actual.
            
            int renderWidth = (int) getWidth();
            int renderHeight = (int) getHeight(); // 32 por defecto
            int renderY = (int) getY();

            if (estado == EstadoDK.AGARRANDO_PRINCESA) {
                renderHeight = (int) HEIGHT_AGARRA_PRINCESA; // 40
                
                // Ajustar 'y' para que el ancla sea la base del sprite.
                // Si el sprite es más alto (40 vs 32), debe dibujarse
                // más arriba (diferencia de 8px)
                renderY = (int) (getY() - (HEIGHT_AGARRA_PRINCESA - HEIGHT));
            }
            
            // Usar las variables renderWidth, renderHeight y renderY
